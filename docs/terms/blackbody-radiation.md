...
`M_n = Σ_i w_i(T_i/T_0)ⁿ` over the `N` surfaces, each carrying its own
solid-angle weight `w_i` (with `Σw_i = 1`):

$$(P-1)_{\text{BBR}} = \frac{\Delta\nu_{\text{stat}}\cdot M_4 + \sum_n c_n M_n}{\nu_0}$$

A uniform, single-surface environment makes every `M_n` equal to
`(T_1/T_0)ⁿ`, so this reduces to the single-temperature formula term for
term.
...
KA5 checks the E32 closed-form polynomial at `T = 300 K` and `T = 250 K`
...
reproduction, a weaker evidentiary class than an independent
reproducibility check. The registry's own polynomial is anchored to this
same JILA value (`docs/validation.md`, `benchmarks/RESULTS.md`).
