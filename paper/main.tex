...
published mm-scale redshift slope. A multi-surface thermal-environment
extension and a quantum-motional systematics package for trapped ions,
second-order Doppler from the atom's own motional state, per-mode
participation and intrinsic-micromotion corrections for a two-ion crystal,
and coherent Ramsey visibility under motional squeezing, extend the
physical model. Four further cases validate these additions: an arithmetic
...
evidentiary class, weaker than this paper's two reproducibility cases,
stated in full where it is discussed. A blind prediction, against data the
...
(W\"ubbena et al.~\cite{Wubbena2012}), an in-phase and an out-of-phase mode
whose equal-mass limit gives $p=1/2$ for both, each ion carrying exactly
half. The two radial pairs additionally depend on trap RF/DC geometry
...
against Marshall et al.'s own published secular-motion band (shaded). WP30:
one species mass carried through every mode. WP31: the two-ion participation
factor from the Al$^+$/Mg$^{25+}$ mass ratio alone (W\"ubbena et
al.~\cite{Wubbena2012}). WP32: the axial participation factor unchanged from
WP31, with the radial pairs reconstructed from the measured X/Y mode
spectrum. WP33: WP32's
participation multiplied by Berkeland's mode-specific intrinsic-micromotion
