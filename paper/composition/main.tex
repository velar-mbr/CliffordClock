...
gravitational redshift), each row computed in its own formalism, the
rows summed linearly, their uncertainties combined in quadrature, and
fringe contrast treated as a separate measurement outside the budget.
We describe an alternative composition already implemented and
validated in the open-source CliffordClock pipeline: every systematic
enters as a multiplicative factor on a single scalar rate at which an
atom's own proper time advances along its worldline, the accumulated
phase is the time integral of that rate, the clock's reported
fractional shift is the first moment of the resulting ensemble phase
distribution, and the Ramsey fringe visibility is the modulus of the
mean phasor, fixed by the second moment (the variance) of that same
distribution under the standard Gaussian-phase closure. The additive
table is the first-order term of this product's own expansion, and we
...
both roughly fifteen orders of magnitude below the $10^{-18}$-$10^{-19}$
systematic floor of the clocks the budget describes. The value of the
product form is structural. It changes no published total: the terms
it adds beyond first order sit fifteen orders of magnitude below
current systematic floors, a bound this paper states directly from
the pipeline's own composition rules. We show three places the structure pays with
numbers already validated in the companion paper: a multi-surface
thermal-environment term whose higher-order moments must be evaluated
per surface before they are combined; a trapped-ion secular-motion
reconciliation where composing two systematics as correlated factors
on one rate closes a discrepancy of $14\sigma$ down to $0.08\sigma$
against a published budget; and a Ramsey visibility read from the
same worldline ensemble that sets the mean shift, computed with no
separate fit. Relativistic timing already composes rate scalars for its
own gravitational and kinematic terms; this paper's own claim is one
of scope: the same composition extended to the atomic-environment
terms of a clock's full budget, evaluated per quantum state, with
fringe contrast recovered as a second moment of the same object, in
an open implementation checked against published budgets. Code,
tests, and the ten-line composition rule itself are public under the
AGPLv3.
\end{abstract}
...
at a time. A group evaluates the blackbody-radiation shift from a
Planck-integral model of the clock's radiative environment, the
lattice or probe AC-Stark shift from the trap-light polarizability, the
DC-Stark shift and its spatial gradient from a field survey, the
electric-quadrupole shift (for an ion with $J\ge1$) from the local
field-gradient tensor and an atomic theory coefficient, the motional
time-dilation shift from sideband-thermometry-measured mode
populations, and the gravitational redshift from a height survey
...
$+0.3(0.2)\times10^{-20}$ per millimetre from a field survey and, in
the same table, a gravitational-redshift slope from two independent
analysis methods that give $-9.8(2.3)\times10^{-20}$ and
$-12.8(2.7)\times10^{-20}$ per millimetre respectively, the two
methods differing from each other by $0.85\sigma$ even though both
...
scalar rate at which an atom's own proper time advances, and the
...
composition rule in ten lines of code.
...
still computed by its own physics: the DC-Stark shift from the
transition's differential polarizability and the local field, the
blackbody shift from a Planck-integral model of the radiative
environment, the quadrupole shift from the local field-gradient
tensor, the motional shift from the atom's own velocity or vibrational
occupation, the gravitational term from height in a local potential.
...
phase distribution's second moment, its variance, the same
distribution whose first moment fixed the shift in
Eq.~\eqref{eq:shift}. One worldline ensemble, run once, yields both
...
in the underlying phases, because the spread is discarded before the
modulus is ever taken. The correct order, Eq.~\eqref{eq:visibility},
...
\section{Equivalence and scope}
\label{sec:equivalence}

Expanding the product in Eq.~\eqref{eq:rate} to first order in each
...
exact cancellation, and their multiplicative product correction, the
...
paper, Sec.~II.E, ``The gravitational-redshift pivot term.''} Both
bounds sit roughly fifteen orders of magnitude below the
...
as it does, and it gives that rule's own quantitative boundary, the
order at which it would stop working, in the same units the rows
themselves are reported in. It does not correct any of the published
totals in Sec.~\ref{sec:intro}: the additive budgets those papers
report are correct to a precision no experiment now approaches, and
nothing in this paper revises a single published number. The three
cases of Sec.~\ref{sec:payoff} instead show where the product form's
other structural consequences change a conclusion: evaluating a
nonlinear function of a distributed quantity before averaging it,
composing two systematics as correlated factors on one shared rate,
and reading a fringe visibility off the same object that sets the
shift. None of the cross-term corrections above ever enters that
arithmetic.

\section{Where the structure pays}
\label{sec:payoff}
...
enclosure whose visible surfaces can differ by tens of kelvin, and
the blackbody shift's dynamic term scales as $T^6$ through $T^{10}$,
higher powers than the $T^4$ power the static shift and the
naturally chosen effective temperature both track. A single
temperature chosen to match the static ($T^4$) moment of a
non-uniform environment does not, in general, reproduce the
...
$M_n = \sum_i w_i (T_i/T_0)^n$, and the static and dynamic terms of
...
$-3.325\times10^{-15}$, against their own closed-form model's
$-3.32(7)\times10^{-15}$ and their own measurement's
$-3.33(3)\times10^{-15}$, landing within $5\times10^{-18}$ of both
their model and their measurement, well inside either
one's own stated uncertainty.\footnote{Companion paper,
...
value stands in for a physically distributed quantity throughout an
optical-clock budget, an effective lattice depth over a trapped
ensemble's spatial extent, an effective field over a cloud's own
spread, and each substitution risks the same failure mode a single
...
distribution comes first, and the moments (the mean that sets the
shift, the variance that sets the visibility) come from integrating
over the full distribution, before any collapse to one representative
value.

\subsection{A cross-row correlation a table cannot see: reconciling a trapped-ion secular-motion budget}
\label{sec:motional}

A trapped two-ion clock's motional time-dilation shift depends on two
physically distinct pieces of the same underlying motion: how much of
each shared vibrational mode the clock ion itself carries (its
participation), and how much extra oscillation, at the trap's own
radio-frequency drive, that secular motion itself induces (intrinsic
micromotion). A budget table built one row at a time can compute
either piece on its own, and Marshall et al.'s published
...
reconciliation, each row built from Marshall et al.'s own published
six-mode frequencies and occupations, reported against Marshall et
al.'s own published secular-motion band of
...
\begin{table}[t]
\centering
\caption{Six successive treatments of the same published Al$^+$
secular-motion total, computed by the CliffordClock pipeline against
Marshall et al.'s own published six-mode inputs~\cite{Marshall2025},
copied from the companion paper's own progression table and
Fig.~8 there. $\sigma$ is the distance from Marshall et al.'s
published band, combining the published and predicted uncertainties
in quadrature. Participation and micromotion enter as two
multiplicative factors on the same per-mode rate (Eq.~\eqref{eq:rate}).}
\label{tab:motional}
\begin{tabular}{lcc}
\toprule
Treatment & $\sigma$ & Verdict \\
\midrule
Single species mass, every mode & 0.10 & MET \\
Participation, mass-ratio closed form & 14.11 & NOT MET \\
Participation, measured-spectrum fit & 14.01 & NOT MET \\
Participation $\times$ leading-order micromotion & 1.62 & NOT MET \\
Participation $\times$ exact-Floquet micromotion & 1.62 & NOT MET \\
Coupled two-ion Floquet, constrained fit & 0.08 & MET \\
\bottomrule
\end{tabular}
\end{table}

The first row carries one species mass through every one of the six
modes, with no participation split and no micromotion correction, and
...
mode is close to one-half, and the true intrinsic-micromotion
enhancement on the radial modes is close to a factor of two; the
single-mass row's implicit choice of one for both factors happens to
land close to their true product, an accident of the approximation
the next two rows expose. Supplying only the participation correction,
rows two and three, removes that accidental cancellation without
replacing it: the resulting total under-predicts the published band
by a factor of close to two, a $14\sigma$ discrepancy against
Marshall et al.'s own stated uncertainty. Composing the
intrinsic-micromotion enhancement back in as a second multiplicative
factor on the same per-mode rate, rows four and five, closes most of that
distance, to $1.62\sigma$, still outside the published band. The
...
$0.08\sigma$, row six, comfortably inside Marshall et al.'s own
published band.

A budget table computes participation and micromotion as separate
rows, or omits the smaller of the two once the first row already
looks close to the published value. Composing them as two
correlated factors on the same per-mode rate exposes the
near-cancellation the single-mass row hides. It also lets the
...
\subsection{Shift and coherence from one object: Ramsey visibility}
...
pipeline run: sweeping a classical worldline ensemble's squeezing
parameter over several values at $4000$ worldlines each, the largest
relative deviation between the pipeline's own reported visibility and
the same run's independently computed Gaussian-closure check lands at
roughly $0.65\%$, at the largest squeezing swept, consistent with the
$O(\sigma^4/N)$ Monte Carlo scaling the pipeline's own test-tolerance
derivation documents for this
...
special-relativistic velocity term, composed multiplicatively along
the satellite's worldline, the same proper-time-rate object this
paper composes for an optical clock's atomic-environment
...
reference, a practice a recent review of atomic clocks for geodesy
surveys across a range of clock platforms and baselines, each one
treating the clock's fractional frequency as a rate factor tied to
...
full budget, including the atomic-environment terms, the DC-Stark
shift, blackbody radiation, and the electric-quadrupole shift, that
have no counterpart in a GPS satellite's or a geodetic baseline's own
rate factor. Those terms are evaluated per quantum state where the
physics calls for it, the per-surface thermal moments of
Sec.~\ref{sec:bbr} and the per-mode motional factors of
Sec.~\ref{sec:motional} among them, and fringe contrast comes out of
that same object, Sec.~\ref{sec:visibility}, as a byproduct of the
shift calculation itself. The implementation is open, and every
...
composed for any of its individual terms.

\section{The composition, in ten lines}
\label{sec:close}

Every equation of Sec.~\ref{sec:law}, the multiplicative rate, the
phase integral, the shift as a first moment, and the visibility as a
second, is this short:
...
from its own formalism exactly as any published budget row is; the
composition itself is the four lines that use them. The source code,
