...
No single source combines the registry species (85Rb), the registry l (D5/2), and coverage through an avoided crossing with printed (non-digitized) numbers (dossier Sec. 2c/6, item 2); the three parts above are the fullest composite this project can support, each labeled with its own evidentiary class, none presented as a full through-the-crossing validation on its own.
